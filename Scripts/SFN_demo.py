...
def read_from_serial(ser, canvas, delta1_text, delta2_text, circle1, circle2,
                     ax, fig, counter1_text, counter2_text,
                     timer1_text, timer2_text, delta1_index, delta2_index, graph_indices):
    ...
    if not hasattr(canvas, 'duke_touch'):
        canvas.duke_touch = tk.PhotoImage(file="duke_touch_resized.png")
    if not hasattr(canvas, 'UH_touch'):
        canvas.UH_touch = tk.PhotoImage(file="UH_touch_resized.png")
        ...
        try:
            ...
            if len(values) == 25:
                ...
                if delta1 > DELTA_THRESHOLD1:
                    # Instead of changing to green, update the image to duke_touch.png
                    canvas.itemconfig(circle1, image=canvas.duke_touch)
                    if not delta1_touch_detected:
                        delta1_touch_detected = True
                        delta1_counter += 1
                        delta1_start_time = time.time()
                elif delta1 <= DELTA_THRESHOLD1:
                    # Revert the image back to the normal duke.png
                    canvas.itemconfig(circle1, image=canvas.duke_img)
                ...
                if delta2 > DELTA_THRESHOLD1:
                    # Update the image to UH_touch.png instead of changing to green
                    canvas.itemconfig(circle2, image=canvas.UH_touch)
                    if not delta2_touch_detected:
                        delta2_touch_detected = True
                        delta2_counter += 1
                        delta2_start_time = time.time()
                elif delta2 <= DELTA_THRESHOLD1:
                    # Revert to the normal image (UH.png)
                    canvas.itemconfig(circle2, image=canvas.UH_img)
                    if delta2_touch_detected:
                        delta2_touch_detected = False
                        delta2_timer += time.time() - delta2_start_time

                # Update the counters and timers on the canvas
                canvas.itemconfig(counter1_text, text=f"Duke Count: {delta1_counter}")
                canvas.itemconfig(counter2_text, text=f"Houston Count: {delta2_counter}")
                canvas.itemconfig(timer1_text, text=f"Duke Timer: {delta1_timer:.2f} sec")
                canvas.itemconfig(timer2_text, text=f"Houston Timer: {delta2_timer:.2f} sec")
...
def initialize_gui(delta1_index, delta2_index):
    ...
    # Load images for touch indicators
    duke_img = tk.PhotoImage(file="duke_resized.png")
    UH_img = tk.PhotoImage(file="UH_resized.png")

    # Keep references to images to avoid garbage collection
    canvas.duke_img = duke_img
    canvas.UH_img = UH_img

    # Replace your ovals with images (keeping the variable names circle1 and circle2)
    # The centers (325,175) and (675,175) are derived from the original ovals' positions.
    circle1 = canvas.create_image(325, 175, image=duke_img, anchor="center")
    circle2 = canvas.create_image(675, 175, image=UH_img, anchor="center")

    # Delta text displays (these will be drawn on top of the images)
    delta1_text = canvas.create_text(325, 175, text="Delta 1: ", font=("Helvetica", 16), state="hidden")
    delta2_text = canvas.create_text(675, 175, text="Delta 2: ", font=("Helvetica", 16), state="hidden")

    # Counter and timer displays moved higher
    counter1_text = canvas.create_text(325, 325, text="Duke Count: 0", font=("Helvetica", 16))
    counter2_text = canvas.create_text(675, 325, text="Houston Count: 0", font=("Helvetica", 16))
    timer1_text = canvas.create_text(325, 375, text="Duke Timer: 0.00 sec", font=("Helvetica", 16))
    timer2_text = canvas.create_text(675, 375, text="Houston Timer: 0.00 sec", font=("Helvetica", 16))
